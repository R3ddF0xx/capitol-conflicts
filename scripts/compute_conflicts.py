...
def get_sector_for_ticker(ticker):
    """Look up the sector for a ticker using a simple yfinance call."""
    try:
...
def fetch_all_members(member_filter=None):
    query = supabase.table("members").select("id")
    if member_filter:
...
def process_member(member_id):
    ...
    for disclosure in disclosures:
        ...
        for mv in member_votes:
            ...
            if not sector_match:
                continue  # skip if no sector overlap at all
    ...
    if conflicts:
        supabase.table("conflicts").upsert(conflicts).execute()
